        ...
        try {
            ...
            for ( int i = 0; i < nodes.getLength(); ++i ) {
                ...
                if ( tmp.getLocalName() != null && tmp.getLocalName().equals("holiday") ) {
                    HolidayConfItem tmpItem = new HolidayConfItem();
                    NodeList holidayElements = tmp.getChildNodes();

                    for ( int n = 0; n < holidayElements.getLength(); ++n ) {
                        Node tmpNode = holidayElements.item(n);
                        ...
                        if ( tmpNode.getLocalName() != null ) {
                            if ( tmpNode.getLocalName().equals("name") ) {
                                ...
                                HolidayTime holidayTime = getTime(tmpNode);
                                if (holidayTime != null) {
...
//                        System.out.println(tmpItem.toString());
    ...
    private HolidayTime getTime(Node timeNode) throws ParseException {
        HolidayTime holidayTime = new HolidayTime();
        NodeList timeNodes = timeNode.getChildNodes();

        for ( int k = 0; k < timeNodes.getLength(); ++k) {
            String nodeName = timeNodes.item(k).getLocalName();
            if ( nodeName != null && nodeName.equals("dayAfterEaster") ) {
                ...
                NodeList fixDateNodes = timeNodes.item(k).getChildNodes();
                ...
                for ( int l = 0; l < fixDateNodes.getLength(); ++l) {
                    if (fixDateNodes.item(l).getLocalName() != null) {
                        if ( fixDateNodes.item(l).getLocalName().equals("date") ) {
                            ...
                            Date time = formatter.parse( fixDateNodes.item(l).getTextContent() );
                            cal.setTime( time );
     ...
     */
    public void setHolidayList(List<HolidayConfItem> holidayList) {
